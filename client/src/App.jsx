...
function App() {
  ...
  useEffect(() => {
    ...
    socket.on('chat_message', (msgData) => {
      setMessages((prev) => [...prev, { type: 'chat', sender: msgData.sender, text: msgData.text }]);
    });
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-gray-900 text-white p-4">
        <div className="bg-gray-800 p-8 rounded-xl shadow-2xl border border-gray-700 text-center w-full max-w-md relative overflow-hidden">
          ...
          <h1 className="text-4xl font-extrabold mb-2 tracking-tight">Mafia Scum</h1>
          <p className="text-gray-400 mb-8 font-medium">A Social Deduction Web Client</p>
  ...
  if (isMod) {
    return <ModView gameState={gameState} timeLeft={timeLeft} socket={socket} messages={messages} />;
  }

  return <PlayerView gameState={gameState} timeLeft={timeLeft} socket={socket} messages={messages} />;
}
