...
export default function ChatBox({ messages, onSendMessage, phase, isBlackmailed, isDead }) {
  ...
  const canChat = (!isDead && !isBlackmailed && (phase === 'DAY_DISCUSSION' || phase === 'LOBBY'));
  ...
  return (
    <div className="flex flex-col h-full bg-gray-800 rounded-lg shadow-lg">
      ...
      <div className="flex-1 p-4 overflow-y-auto max-h-[500px] min-h-[300px]">
        {messages.map((m, i) => (
          <div key={i} className={`mb-2 text-sm md:text-base ${m.type === 'system' ? 'text-yellow-400 italic font-semibold' : 'text-gray-200'}`}>
            {m.type === 'system' ? 
              `[System] ${m.text}` : 
              <span className="break-words"><strong>{m.sender}: </strong>{m.text}</span>
            }
          </div>
        ))}
      </div>
      <div className="p-4 bg-gray-900 rounded-b-lg">
        {isBlackmailed && phase !== 'LOBBY' && <p className="text-red-500 text-sm mb-2 font-bold animate-pulse">You are blackmailed. You cannot speak today.</p>}
        {isDead && <p className="text-gray-500 text-sm mb-2 italic">You are dead. The living cannot hear you.</p>}
        <form onSubmit={handleSubmit} className="flex gap-2">
          <input
            ...
            placeholder={canChat ? "Type your message..." : "You cannot chat right now."}
            value={chatInput}
