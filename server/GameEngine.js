...
class GameEngine {
  ...
  resetGame() {
    ...
    this.io.emit('system_message', "Game has been reset to Lobby by the Moderator.");
    this.broadcastState();
  }
  ...
  startPreGame(rolesList) {
    ...
    if (playerIds.length !== rolesList.length) {
      console.error(`Role list length mismatch. Expected ${playerIds.length}, got ${rolesList.length}`);
    }
  ...
  handleModAction(data) {
    if (data.action === 'pause') {
      ...
      this.io.emit('system_message', this.isPaused ? "Game Paused by Moderator." : "Game Resumed.");
    } else if (data.action === 'skip') {
      if (this.timer) clearInterval(this.timer);
      this.timeLeft = 0;
      if (this.phase === 'NIGHT') this.transitionPhase('DAY_ANNOUNCEMENT');
      else if (this.phase === 'DAY_ANNOUNCEMENT') this.transitionPhase('DAY_DISCUSSION');
      else if (this.phase === 'DAY_DISCUSSION') this.transitionPhase('DAY_VOTING');
  ...
  handleChatMessage(socketId, text) {
    ...
    if (this.phase === 'NIGHT') {
      if (player.team === TEAM_MAFIA) {
        const mafiaIds = Object.values(this.players).filter(p => p.team === TEAM_MAFIA).map(p => p.id);
        mafiaIds.forEach(id => {
          this.io.to(id).emit('chat_message', { sender: `[Mafia] ${player.username}`, text });
        });
      }
      return;
    }
  ...
  submitDayVote(socketId, targetId) {
    ...
    this.io.to(socketId).emit('system_message', `You voted for ${target.username}.`);
  }

  resolveDayVoting() {
    console.log("Resolving day votes...", this.dayVotes);
    if (Object.keys(this.dayVotes).length === 0) {
      this.io.emit('system_message', "No one was voted up today. Town decides not to execute anyone.");
      return;
    }
    ...
    if (tie || !executedId) {
      this.io.emit('system_message', "The town was split in a tie. No one will be executed today.");
    } else {
  ...
  resolveNight() {
    ...
    for (const [actorId, targetId] of Object.entries(this.nightActions)) {
      ...
      if (role.name === 'Veteran' && targetId === actorId && actor.veteranAlerts > 0) {
        alerts.add(actorId);
        actor.veteranAlerts--;
        messages[actorId].push("You went on alert. Anyone who visits you will be shot!");
        continue; 
      }

      // If they target someone else or valid action, push to queue
      if (targetId && target) {
        actions.push({ actor, target, targetId, priority: role.priority || 99, role });
      }
    }
    ...
    for (const action of actions) {
      ...
      if (target && alerts.has(target.id) && target.id !== actor.id) {
        nightKills.push({ attackerId: target.id, targetId: actor.id, type: 'Veteran' });
      ...
      switch (role.name) {
        ...
        case 'Consort':
          ...
          messages[targetId].push("Someone occupied your night. You were roleblocked.");
          break;
        case 'Doctor':
        ...
        case 'Cult Leader':
          ...
          if (target && (target.team === TEAM_MAFIA || role.name === 'Serial Killer' || target.role.name === 'Serial Killer')) {
            messages[actor.id].push("Your target was immune to your conversion.");
          } else if (target) {
            culted.add(targetId);
            messages[actor.id].push("You successfully converted your target.");
            messages[targetId] = messages[targetId] || [];
            messages[targetId].push("You have been converted! You are now a Cultist.");
          }
          break;
        case 'Framer':
          framed.add(targetId);
        ...
        case 'Blackmailer':
          ...
          messages[targetId] = messages[targetId] || [];
          messages[targetId].push("You have been blackmailed. You cannot speak tomorrow.");
          break;
        case 'Godfather':
        case 'Mafioso':
        ...
        case 'Vigilante':
          if (actor.vigilanteBullets > 0) {
            ...
            nightKills.push({ attackerId: actor.id, targetId, type: 'Vigilante' });
          }
          break;
        case 'Consigliere':
          if (target) {
             messages[actor.id].push(`Your target is exactly a ${target.role.name}.`);
          }
          break;
        case 'Detective':
          if (target) {
            ...
            messages[actor.id].push(isMafia ? "Your target appears to be Mafia!" : "Your target appears innocent.");
          }
          break;
        case 'Tracker':
          if (target) {
    ...
    if (gfKill && !blocks.has(gfKill.actor.id)) {
      ...
      mafiaKillerId = (mfKill && !blocks.has(mfKill.actor.id)) ? mfKill.actor.id : gfKill.actor.id;
    ...
    if (mafiaTarget) {
      nightKills.push({ attackerId: mafiaKillerId, targetId: mafiaTarget, type: 'Mafia' });
    }
    ...
    for (const kill of nightKills) {
      ...
      if ((kill.type === 'Mafia' || kill.type === 'SK') && heals.has(kill.targetId)) {
        messages[kill.targetId] = messages[kill.targetId] || [];
        messages[kill.targetId].push("You were attacked but someone nursed you back to health!");
        continue;
      }
      ...
      if (kill.type === 'Vigilante' && victim.team === TEAM_TOWN && victim.role.name !== 'Mayor') {
    ...
    Object.values(this.players).forEach(p => {
      ...
      if (p.isAlive && p.role.name === 'Executioner' && deaths.has(p.executionerTarget)) {
        p.role = Object.values(ROLES).find(r => r.name === 'Jester');
        messages[p.id] = messages[p.id] || [];
        messages[p.id].push("Your target died at night. You have gone mad and become a Jester!");
      }
    ...
    Object.values(this.players).forEach(p => {
      if (deaths.has(p.id)) {
        graveyardDetails.push(`${p.username} died last night. They were a ${p.role.name}.`);
      }
    });

    if (graveyardDetails.length === 0) {
      this.io.emit('system_message', "The town woke up to find everyone alive, remarkably.");
    } else {
  ...
  checkWinConditions() {
    ...
    if (totalAlive === 0) {
      this.declareWinner('Draw', 'The town is completely wiped out. No one survived.');
      return;
    }
    ...
    if (aliveSK > 0 && aliveMafia === 0 && aliveTown === 0 && aliveCult === 0) {
      this.declareWinner('Serial Killer', 'The Serial Killer has successfully murdered the entire town.');
      return;
    }

    // 3. Mafia Win Condition -> No remaining threats (Town, SK, Cult dead)
    if (aliveMafia > 0 && aliveTown === 0 && aliveSK === 0 && aliveCult === 0) {
      this.declareWinner('Mafia', 'The Mafia has eliminated all resistance and controls the town.');
      return;
    }

    // 4. Town Win Condition -> All criminals/eval doers are eradicated
    if (aliveTown > 0 && aliveMafia === 0 && aliveSK === 0 && aliveCult === 0) {
      this.declareWinner('Town', 'The Town has successfully rooted out all evildoers!');
      return;
    }
  }
  ...
  executePlayer(playerId) {
    ...
    this.io.emit('system_message', `${executedPlayer.username} was executed by the town. They were a ${executedPlayer.role.name}.`);

    // Edge Case: Jester Win Condition
    if (executedPlayer.role.name === 'Jester') {
      this.io.emit('system_message', `Jester Win: ${executedPlayer.username} has tricked the town into executing them!`);
    ...
    Object.values(this.players).forEach(p => {
      if (p.role && p.role.name === 'Executioner' && p.executionerTarget === playerId) {
        this.io.emit('system_message', `Executioner Win: ${p.username} successfully got their target executed!`);
      }
    });
  ...
  broadcastState() {
    ...
    Object.values(this.players).forEach(p => {
      ...
      const isOmniscient = p.username === 'Admin/Mod' || this.phase === 'GAME_OVER';
